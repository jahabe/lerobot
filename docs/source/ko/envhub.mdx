# Hub에서 환경(Environment) 불러오기

**EnvHub** 기능을 사용하면 코드 한 줄만으로 Hugging Face Hub에서 시뮬레이션 환경을 직접 불러올 수 있습니다. 이를 통해 강력한 협업 방식이 가능해집니다. 환경이 대형 라이브러리 안에 갇혀 있지 않고, 누구나 커스텀 환경을 게시하고 커뮤니티와 공유할 수 있습니다.

## EnvHub란?

EnvHub를 사용하면 원하는 로봇 모델과 시나리오로 커스텀 로보틱스 시뮬레이션 환경을 만들고, LeRobot 프레임워크를 통해 누구나 쉽게 사용할 수 있도록 공유할 수 있습니다.

EnvHub 패키지는 Hugging Face Hub에 Git 저장소 형태로 저장되며, 코드 한 줄로 LeRobot AI 로보틱스 프로젝트에 가져와 바로 사용할 수 있습니다.

EnvHub를 활용하면 다음이 가능합니다:

1. **환경 생성 및 게시**: Hugging Face Hub에 Git 저장소로 업로드하고, 복잡한 패키징 과정 없이 물리 시뮬레이션을 배포할 수 있습니다.
2. **환경 동적 로드**: 패키지를 별도로 설치하지 않고도 불러올 수 있습니다.
3. **버전 관리 및 추적**: Git 방식으로 환경 변경 이력을 관리할 수 있습니다.
4. **탐색**: 커뮤니티가 공유한 새로운 시뮬레이션 태스크를 검색할 수 있습니다.

이 구조 덕분에 Hub에서 흥미로운 환경을 발견한 즉시 실험을 시작하거나, 의존성 충돌이나 복잡한 설치 과정 걱정 없이 커스텀 로봇과 환경을 직접 만들 수 있습니다.
