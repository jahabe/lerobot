...
def make_env(n_envs: int = 1, use_async_envs: bool = False):
    """
    커스텀 태스크용 백터화 환경을 생성합니다. 

    Args:
        n_envs: 병렬 환경 수 
        use_async_envs: AsyncVectorEnv 또는 SyncVectorEnv 사용 여부

    Returns:
        gym.vector.VectorEnv 또는 suite 이름을 백터화 환경에 매핑하는 딕셔너리
    """
...
또는 `huggingface_hub` Python API를 사용할 수도 있습니다:
...
여러 환경을 병렬로 실행할 때 성능을 높이려면:

```python
envs_dict = make_env(
    "username/my-env",
    n_envs=8,
    use_async_envs=True,  # 병령 실행을 위해 AsyncVectorEnv 사용
    trust_remote_code=True
)
```
...
허브 URL 형식은 다음 패턴을 지원합니다:

| 패턴                  | 설명                           | 예시                                |
| -------------------- | ------------------------------ | -------------------------------------- |
| `user/repo`          | 메인 브랜치에서 `env.py` 불러오기 | `make_env("lerobot/pusht-env")`        |
| `user/repo@revision` | 특정 리비전에서 불러오기          | `make_env("lerobot/pusht-env@main")`   |
| `user/repo:path`     | 커스텀 파일 불러오기              | `make_env("lerobot/envs:pusht.py")`    |
| `user/repo@rev:path` | 리비전 + 커스텀 파일             | `make_env("lerobot/envs@v1:pusht.py")` |
